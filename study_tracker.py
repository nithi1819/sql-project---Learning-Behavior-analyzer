...
# ── DB CONNECTION — fill in your details ─────────────────────────────────────
DB_USER     = "msc24pt30"
...
def db_total_hours():
    with get_conn() as conn:
        with conn.cursor() as cur:
            result = cur.callfunc("total_time_per_week", oracledb.DB_TYPE_NUMBER, [])
            return int(result or 0)

def db_avg_score(subj_id):
    with get_conn() as conn:
        with conn.cursor() as cur:
            result = cur.callfunc("avg_score_per_subject", oracledb.DB_TYPE_NUMBER, [subj_id])
            return float(result or 0)

def db_avg_time(subj_id):
    with get_conn() as conn:
        with conn.cursor() as cur:
            result = cur.callfunc("avg_time_per_subject", oracledb.DB_TYPE_NUMBER, [subj_id])
            return float(result or 0)

def db_avg_gap(subj_id):
    with get_conn() as conn:
        with conn.cursor() as cur:
            result = cur.callfunc("gap_between_sessions_per_subject", oracledb.DB_TYPE_NUMBER, [subj_id])
            return float(result or 0)

def db_freq(subj_id):
    with get_conn() as conn:
        with conn.cursor() as cur:
            result = cur.callfunc("freq_sessions_per_subject", oracledb.DB_TYPE_NUMBER, [subj_id])
            return int(result or 0)
...
def db_insights():
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            cur.callproc("main", [])
            lines = []
            # Moved variables outside the loop for efficiency
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Student Performance Analytics Dashboard — Nithyasri")
    ...
    def _build(self):
        ...
        tr.pack(fill="x")
        tk.Label(tr, text="Student Performance Analytics Dashboard — Nithyasri",
                 font=("Georgia", 18, "bold"), fg=FG, bg=BG).pack(side="left")
        tk.Label(tr, text="semester · 2026",
    ...
    def _build_subj(self):
        ...
        hdr.pack(fill="x")
        for lbl, w, anc in [
            ("subject",200,"w"),("difficulty",90,"center"),
    ...
    def _fill_subj(self, subjects):
        ...
        for i, (subj_id, name, diff, ca1, ca2) in enumerate(subjects):
            ...
            tk.Label(row, text=name, font=("Helvetica",10,"bold"),
                     fg=FG, bg=rbg, anchor="w", padx=10, pady=10, width=18).pack(side="left")
